...
{
  //Variable Definition
  vector<double> x,xdot, S, points, time ;
  ...
  for(int i=1; i<=n; i++)
  {
    ...
    x.push_back((points[i]+points[i-1])/2); //medium point of the interval
    S.push_back(deltat*(lagrangian->Eval(x[i],xdot[i],0))); //action in each small interval
  ...
  for(int i=0; i<n; i++)
  {
    ...
    for(int j=1;j<100;j++) //increases values of position change
    {
      ...
      if(S[i]+S[i+1]>aux)
      {
        ...
        S[i]=deltat*(lagrangian->Eval(x[i],xdot[i],0));
        S[i+1] = deltat*(lagrangian->Eval(x[i+1],xdot[i+1],0));
...
/*Preparar grafico com plot dos pontos tais que points é um vector com os valores no eixo do y (posição) e time é um vector
 com os valores no eixo do x. Dá nos a trajectória que vai ter que se comparar com o plot da equação do movimento*/
